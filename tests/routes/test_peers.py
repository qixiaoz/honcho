...
def test_delete_peer_endpoint(client: TestClient, sample_data: tuple[Workspace, Peer]):
    ...
    deleted = client.delete(f"/v3/workspaces/{workspace.name}/peers/{peer_name}")
    assert deleted.status_code == 202
    payload = deleted.json()
    assert payload["peer"] == peer_name
    assert payload["workspace"] == workspace.name

    deleted_again = client.delete(f"/v3/workspaces/{workspace.name}/peers/{peer_name}")
